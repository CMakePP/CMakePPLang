...
function(cpp_contains _cc_result _cc_item _cc_list)
    ...
    if("${_cc_list_type}" STREQUAL "array")
        cpp_array(END _cc_length "${_cc_list}")
        if("${_cc_length}" STREQUAL "-1")
            cpp_return("${_cc_result}")
        endif()

        foreach(_cc_i RANGE "${_cc_length}")
            cpp_array(GET _cc_arg_i "${_cc_list}" "${_cc_i}")
            cpp_equal(_cc_are_equal "${_cc_item}" "${_cc_arg_i}")
            if("${_cc_are_equal}")
                set("${_cc_result}" TRUE PARENT_SCOPE)
                return()
            endif()
        endforeach()
    elseif("${_cc_list_type}" STREQUAL "list")
        if("${_cc_item}" IN_LIST _cc_list)
            set("${_cc_result}" TRUE PARENT_SCOPE)
        endif()
    elseif("${_cc_list_type}" STREQUAL "map")
        cpp_map(HAS_KEY "${_cc_result}" "${_cc_list}" "${_cc_item}")
        cpp_return("${_cc_result}")
    elseif("${_cc_list_type}" STREQUAL "desc")
        string(FIND "${_cc_list}" "${_cc_item}" _cc_pos)
        if(NOT "${_cc_pos}" STREQUAL "-1")
            set("${_cc_result}" TRUE PARENT_SCOPE)
        endif()
    endif()
endfunction()
