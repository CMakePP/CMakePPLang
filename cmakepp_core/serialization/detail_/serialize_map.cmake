...
function(_cpp_serialize_map _csm_return _csm_handle)
    ...
    foreach(_csm_i ${_csm_keys})
        if(_csm_not_1st)
            set("${_csm_return}" "${${_csm_return}} ,")
        endif()
       cpp_map(GET _csm_value "${_csm_handle}" "${_csm_i}")
        _cpp_serialize_value(_csm_str "${_csm_value}")
        set("${_csm_return}" "${${_csm_return}} \"${_csm_i}\" : ${_csm_str}")
        set(_csm_not_1st TRUE)
    endforeach()
    set("${_csm_return}" "${${_csm_return}} }" PARENT_SCOPE) # Adds ending "}"
endfunction()
