...
bug). Unfortunately, this means that all other functions intended to implement
CMakePPLang functions need to have their type-safety checked manually. This
section focuses on how to unit test the signatures of functions which are
strongly typed, but are not CMakePPLang functions.
